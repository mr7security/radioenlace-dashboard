#!/usr/bin/env python3
"""
Dashboard de Radioenlaces - proxy de datos
==========================================

Sondea periodicamente los radios Mimosa (serie B / C) leyendo su endpoint
interno /cgi/dashboard.php, normaliza las metricas de RF, guarda un historico
en SQLite y sirve el dashboard web.

Solo usa libreria estandar de Python 3 (no hace falta pip install).

Uso:
    python3 proxy.py                    # usa radios.json si existe
    python3 proxy.py --once             # una sola lectura por consola (diagnostico)
    python3 proxy.py --fixture f.json   # prueba el parser con un JSON guardado

NOTA DE SEGURIDAD: el endpoint del radio devuelve tambien credenciales
(passphrase WPA, hash de admin, community SNMP). Este proxy IGNORA por completo
el bloque 'config': nunca se guarda en la base de datos ni se envia al navegador.
"""
...
def evaluar_estado(m):
    ...
    if peor is not None and peor > -20:
        nivel = subir("aviso")
        avisos.append("EVM pobre en alguna cadena (%.1f dB)" % peor)

    # Desequilibrio entre cadenas: sintoma tipico de una cadena/antena tocada
...
def crear_handler(monitor):
    class Handler(BaseHTTPRequestHandler):
        ...
        def do_GET(self):  # noqa: N802
            ...
            if ruta in ("/", "/index.html", "/dashboard.html"):
                try:
                    with open(HTML_PATH, "rb") as fh:
                        self._responder(200, fh.read(), "text/html; charset=utf-8")
                except FileNotFoundError:
                    self._responder(404, b"Falta dashboard.html", "text/plain; charset=utf-8")
                return
...
def main():
    ap = argparse.ArgumentParser(description="Dashboard de radioenlaces Mimosa")
    ap.add_argument("--once", action="store_true", help="una lectura y salir")
    ap.add_argument("--fixture", help="parsear un JSON guardado en vez de consultar el radio")
    args = ap.parse_args()
